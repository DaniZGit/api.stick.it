-- +goose Up
-- users
ALTER TABLE users ADD CONSTRAINT users_avatar_id_avatars_id FOREIGN KEY (avatar_id) REFERENCES avatars(id);
...
ALTER TABLE user_stickers ADD CONSTRAINT user_stickers_unique UNIQUE(user_id, sticker_id);
ALTER TABLE user_stickers ADD CONSTRAINT user_stickers_amount_positive CHECK (amount >= 0);
...
ALTER TABLE user_packs ADD CONSTRAINT user_packs_amount_positive CHECK (amount >= 0);
...
-- +goose Down
ALTER TABLE users DROP CONSTRAINT IF EXISTS users_avatar_id_avatars_id;
...
ALTER TABLE user_stickers DROP CONSTRAINT IF EXISTS user_stickers_unique;
ALTER TABLE user_stickers DROP CONSTRAINT IF EXISTS user_stickers_amount_positive;
...
ALTER TABLE user_packs DROP CONSTRAINT IF EXISTS user_packs_amount_positive;
